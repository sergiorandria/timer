...
TimerApplication::TimerApplication() : main_loop(nullptr) {
    setup_ui();
}
...
void TimerApplication::setup_ui() {
    // Create main window
...
void TimerApplication::on_window_destroy(GtkWidget *widget, gpointer data) {
...
void TimerApplication::on_start_clicked(GtkWidget *widget, gpointer data) {
    TimerApplication *app = static_cast<TimerApplication*>(data);
    app->start_timer();
}

void TimerApplication::on_pause_clicked(GtkWidget *widget, gpointer data) {
    TimerApplication *app = static_cast<TimerApplication*>(data);
    app->pause_timer();
}

void TimerApplication::on_reset_clicked(GtkWidget *widget, gpointer data) {
    TimerApplication *app = static_cast<TimerApplication*>(data);
    app->reset_timer();
}

void TimerApplication::start_timer() {
    if (!running) return;
...
void TimerApplication::pause_timer() {
    std::lock_guard<std::mutex> lock(mtx);
    if (!paused) {
        paused = true;
...
void TimerApplication::reset_timer() {
    std::lock_guard<std::mutex> lock(mtx);
...
void TimerApplication::update_loop() {
    while (running) {
        ...
        timer.__nanosleep((const struct timespec *)&ts, nullptr);
    }
}

void TimerApplication::update_display(std::chrono::nanoseconds remaining) {
